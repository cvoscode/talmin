...
class BoxPlotter(Base_Fig):
    ...
    def update_fig(self):
        if self.x:
            self.fig=px.box(self.data.to_pandas(),x=self.x,color=self.color, notched=True,points=self.points)
        else:
            self.fig=px.box(self.data.to_pandas(),y=self.y,color=self.color, notched=True,points=self.points)
        if self.title:
    ...
    def get_settings_ui(self):
        ui=dbc.Row([
            ...
            dbc.Col([
            create_dropdown_paging(id=f'points-{self.id}',options=['all','outliers',False],value=None,name='Points',multi=False),
            create_Tooltip('Select the way points are added to the plot',target=f'Points-{self.id}')
            ]),
            dbc.Col([
            dbc.InputGroup([
